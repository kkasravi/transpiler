...
Identifier "identifier"
  = !ReservedWord name:IdentifierName { return ast.Identifier({
       loc: ast.SourceLocation({source:input,start:ast.Position({line:pos0}),end:ast.Position({line:pos})}),
       type:"Identifier",
       name:name
    }); }
...
PrivateIdentifier "privateidentifier"
  = !ReservedWord name:PrivateIdentifierName { return ast.PrivateIdentifier({
       loc: ast.SourceLocation({source:input,start:ast.Position({line:pos0}),end:ast.Position({line:pos})}),
       type:"PrivateIdentifier",
       name:name
    }); }

PrivateIdentifierName "identifier"
  = start:"@" parts:IdentifierPart* {
...
NullLiteral
  = NullToken { return ast.NullLiteral({ 
      type: "NullLiteral" 
    }); }

BooleanLiteral
  = TrueToken  { return ast.BooleanLiteral({ 
      type: "BooleanLiteral", value: true  
    }); }
  / FalseToken { return ast.BooleanLiteral({ 
...
CallExpression
  = base:(
    ...
    argumentsOrAccessors:(
        __ args:Arguments {
          return ast.FunctionCallArguments({
            type: "FunctionCallArguments",
            args: args
          });
        }
      / __ "[" __ name:Expression __ "]" {
          return ast.PropertyAccessProperty({
...
ExportMappings
  = head:ExportMapping tail:(__ "," __ ExportMapping)* {
      var result = [head];
      for (var i = 0; i < tail.length; i++) {
        result.push(tail[i][3]);
      }
...
ExportMapping
  = name:ExportSpecifierSet __ "from" __ value:Expression {
      return ast.ExportMapping({
        loc: ast.SourceLocation({source:input,start:ast.Position({line:pos0}),end:ast.Position({line:pos})}),
        type:  "ExportMapping",
        name:  name,
...
ExportSpecifierSet
  = Identifier
  / "{" __ exports:(ExportSpecifierList __ ("," __)?)? "}" {
      return ast.Exports({
        loc: ast.SourceLocation({source:input,start:ast.Position({line:pos0}),end:ast.Position({line:pos})}),
        type: "Exports",
...
ExportSpecifierList
  = head:ExportSpecifier tail:(__ "," __ ExportSpecifier)* {
      var result = [head];
      for (var i = 0; i < tail.length; i++) {
        result.push(tail[i][3]);
      }
...
ExportSpecifier
  = name:Identifier alias:(":" __ Identifier)? {
      return ast.ExportSpecifier({
        type: "ExportSpecifier",
        alias: alias ? alias[2] : null,
        name: name
      });
    }
...
ExportStatement
  = ExportToken __ declarations:ExportDeclaration EOS {
      return ast.ExportStatement({
        loc: ast.SourceLocation({source:input,start:ast.Position({line:pos0}),end:ast.Position({line:pos})}),
        type: "ExportStatement",
        declarations: Array.isArray(declarations) ? declarations : [declarations]
      });
    }
...
DebuggerStatement
  = DebuggerToken EOS { return ast.DebuggerStatement({ type: "DebuggerStatement" }); }

/* ===== A.5 Functions and Programs ===== */

ClassDeclaration
  = ClassToken __ name:Identifier? __ 
    heritage:Heritage? __
    "{" __ elements:ClassBody __ "}" {
...
Heritage
  = __ inheritance:"extends" __ name:MemberExpression { 
  ...
  / __ inheritance:"prototype" __ name:MemberExpression {
      return ast.Inheritance({ 
        type: "Inheritance", 
        name:name, 
        inheritance:inheritance 
...
ModuleStatement
  = ModuleToken __ declarations:ModuleDeclarationList EOS {
...
ModuleDeclarationList
  = head:ModuleDeclaration tail:(__ "," __ ModuleDeclaration)* {
      var result = [head];
      for (var i = 0; i < tail.length; i++) {
        result.push(tail[i][3]);
      }
...
ModuleDeclaration
  = name:Identifier __ "from" __ expression:ModuleExpression {
...
ModuleExpression
  = root:ModuleReference parts:(__ "." __ IdentifierName)* {
      var nameparts = [root];
      for (var i = 0; i < parts.length; i++) {
        nameparts.push(parts[i][3]);
      }
      return ast.ModuleExpression({
        loc: ast.SourceLocation({source:input,start:ast.Position({line:pos0}),end:ast.Position({line:pos})}),
...
ModuleDefinition
  = ModuleToken __ name:Identifier __
    "{" __ elements:ModuleBody __ "}" {
...
ClassBody
  = elements:ClassElements? { return elements !== "" ? elements : []; }
...
Constructor
  = ConstructorToken __ 
    "(" __ params:FormalParameterList? __ ")" __
    "{" __ elements:ConstructorBody __ "}" {
...
ConstructorBody
  = elements:ConstructorElements? { return elements !== "" ? elements : []; }

ConstructorElements
  = head:ConstructorElement tail:(__ ConstructorElement)* {
      var result = [head];
      for (var i = 0; i < tail.length; i++) {
        result.push(tail[i][1]);
      }
...
PublicPropertyDefinition
  = PublicToken __ property:PropertyDefinition __ {
      return ast.PublicPropertyDefinition({
        loc: ast.SourceLocation({source:input,start:ast.Position({line:pos0}),end:ast.Position({line:pos})}),
        type: "PublicPropertyDefinition",
        property: property
      });
    }

PrivatePropertyDefinition
  = PrivateToken __ property:PropertyDefinition __ {
      return ast.PrivatePropertyDefinition({
        loc: ast.SourceLocation({source:input,start:ast.Position({line:pos0}),end:ast.Position({line:pos})}),
...
StaticPropertyDefinition
  = StaticToken __ property:PropertyDefinition __ { 
      return ast.StaticPropertyDefinition({
        loc: ast.SourceLocation({source:input,start:ast.Position({line:pos0}),end:ast.Position({line:pos})}),
        type:     "StaticPropertyDefinition",
        property: property
      });
    }
...
MethodDefinition
  = __ name:Identifier __ "(" __ params:FormalParameterList? __ ")" __
    "{" __ elements:FunctionBody __ "}" {
...
ModuleBody
  = elements:ModuleElements? { return elements !== "" ? elements : []; }
...
ModuleElements
  = head:ModuleElement tail:(__ ModuleElement)* {
      var result = [head];
      for (var i = 0; i < tail.length; i++) {
        result.push(tail[i][1]);
      }
...
ImportDeclaration
  = name:ImportSpecifierSet __ "from" __ value:Expression {
...
ImportSpecifierSet
  = name:ImportAllToken {
      return ast.ImportIdentifier({
        type:  "ImportIdentifier",
        name:  name
      });
    }
  / Identifier
  / "{" __ imports:(ImportSpecifierList __ ("," __)?)? "}" {
      return ast.Imports({
        loc: ast.SourceLocation({source:input,start:ast.Position({line:pos0}),end:ast.Position({line:pos})}),
        type: "Imports",
